...
const ShoeDetails = () => {
    ...
    const { brand, route } = useParams(); // Get brand and route from URL params
    // Find the shoe data based on brand and route
    const shoe = items.find(
        (item) =>
    ...
    const [displayedImage, setDisplayedImage] = useState(shoe.img);
    const [enlargedImage, setEnlargedImage] = useState(null);
    const closeEnlargedImage = () => {
        setEnlargedImage(null);
    };

    // Update displayedImage whenever the shoe prop changes
    useEffect(() => {
        setDisplayedImage(shoe.img);
    }, [shoe.img]);
    ...
    return (
        <>
            ...
            >
                <p className="notify-text">{shoe.description} added to your cart &nbsp;✅</p>
            </div>
            <div className="motto"><h3>{shoe.brand}</h3></div>
            <Container style={{ padding: '48px 0' }}>
                <Row>
                    <Col lg="2" xs="4">
                        <img src={shoe.img}
                            onClick={() => imageClick(shoe.img, false)}
                            style={{
                                width: "100%",
                                height: "auto",
                                maxHeight: "140px",
                    ...
                    <Col lg="4" xs="12">
                        <Row>
                            <Col xs="12">
                                <h4 className="detail-description">{shoe.description}</h4>
